...
set.seed(71)
train_grid = expand.grid(alpha = 0, lambda = 10^ (3:5 * -1))
...
ridge_predict_train_res = predict(ridge_fit, dtrain[, 3:49], type="prob")
ridge_predict_test_res = predict(ridge_fit, dtest[, 2:48], type="prob")

write_csv(as.data.frame(ridge_predict_train_res$X2), "train_pred_targets_ridge.csv")
write_csv(as.data.frame(ridge_predict_test_res$X2), "test_pred_targets_ridge.csv")
