...
dtrain <- xgb.DMatrix(data=data.matrix(x_train[, 2:65]), label=x_train$target)

dtrain1 <- xgb.DMatrix(data=data.matrix(x_train1[, 2:65]), label=x_train1$target)
dval1 <- xgb.DMatrix(data=data.matrix(x_val1[, 2:65]), label=x_val1$target)
dtrain2 <- xgb.DMatrix(data=data.matrix(x_train2[, 2:65]), label=x_train2$target)
dval2 <- xgb.DMatrix(data=data.matrix(x_val2[, 2:65]), label=x_val2$target)
dtrain3 <- xgb.DMatrix(data=data.matrix(x_train3[, 2:65]), label=x_train3$target)
dval3 <- xgb.DMatrix(data=data.matrix(x_val3[, 2:65]), label=x_val3$target)
dtest <- xgb.DMatrix(data=data.matrix(dtest[, 2:65]))
...
##max.depth = 4.0000	min_child_weight = 9.0000	subsample = 0.7665
##colsample_bytree = 0.6081	gamma = 0.7969	alpha = 0.6848	lambda = 0.7867	Value = 0.6420 
...
dtrain4 <- xgb.DMatrix(data=data.matrix(x_train1[, 2:65]), label=x_train1$target)
dval4 <- xgb.DMatrix(data=data.matrix(x_val1[, 2:65]), label=x_val1$target)
dtrain5 <- xgb.DMatrix(data=data.matrix(x_train2[, 2:65]), label=x_train2$target)
dval5 <- xgb.DMatrix(data=data.matrix(x_val2[, 2:65]), label=x_val2$target)
dtrain6 <- xgb.DMatrix(data=data.matrix(x_train3[, 2:65]), label=x_train3$target)
dval6 <- xgb.DMatrix(data=data.matrix(x_val3[, 2:65]), label=x_val3$target)
...
dtrain7 <- xgb.DMatrix(data=data.matrix(x_train1[, 2:65]), label=x_train1$target)
dval7 <- xgb.DMatrix(data=data.matrix(x_val1[, 2:65]), label=x_val1$target)
dtrain8 <- xgb.DMatrix(data=data.matrix(x_train2[, 2:65]), label=x_train2$target)
dval8 <- xgb.DMatrix(data=data.matrix(x_val2[, 2:65]), label=x_val2$target)
dtrain9 <- xgb.DMatrix(data=data.matrix(x_train3[, 2:65]), label=x_train3$target)
dval9 <- xgb.DMatrix(data=data.matrix(x_val3[, 2:65]), label=x_val3$target)
...
sub <- data.table(id, target=preds1)
write.table(sub, "xgboost_stratifiedCV_tsnesBopt_pred1.csv", sep=",", dec=".", quote=FALSE, row.names=FALSE)

sub <- data.table(id, target=preds)
write.table(sub, "xgboost_stratifiedCV_tsnesBopt_mix9.csv", sep=",", dec=".", quote=FALSE, row.names=FALSE)
