...
set.seed(71)
dtrain2 <- missRanger(dtrain[ ,3:ncol(dtrain)], pmm.k = 3, num.trees = 1000, seed = 71)
dtest2 <- missRanger(dtest[, 2:ncol(dtest)], pmm.k = 3, num.trees = 1000, seed = 71)
...
STCV <- strrandsamplingcrossval(dtrain$target, nfolds = 3, seedno = 71)
...
lgb_cv_bayes <- function(max_depth, 
                         num_leaves, 
                         feature_fraction, 
                         min_gain_to_split,
                         bagging_fraction) {
  cv <- lgb.cv(params = list(objective = "binary",  boosting_type = "gbdt", 
                             learning_rate = 0.003, tree_learner = "serial",
                             metric="auc",
                             max_depth = max_depth,
               ...
               data = dtrainreflgb, nrounds = 1000, 
               folds = cv_folds, early_stopping_rounds = 50, verbose = 0,
               categorical_feature = c(3, 5:12, 14:16, 20:30)
  )
  list(Score = max(cv$record_evals$valid$auc$eval[[1]]))
}

OPT_Res <- BayesianOptimization(lgb_cv_bayes,
                                bounds = list(max_depth = c(8L, 11L),
                                              num_leaves = c(512L, 2048L),
...
#Round = 14	max_depth = 11.0000	num_leaves = 639.0000
#feature_fraction = 0.8000	bagging_fraction = 0.6203	
#min_gain_to_split = 2.4061	Value = 0.6051 
...
params <- list(objective = "binary",  
               ...
               max_depth = 11,
               num_leaves = 639, 
               min_gain_to_split =  2.4061,
               feature_fraction = 0.8, 
               bagging_fraction = 0.6203,
