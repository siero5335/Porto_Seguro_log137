...
set.seed(71)
train_grid = expand.grid(k = c(3, 5, 10, 20, 50))
...
knn_predict_train_res = predict(extree_fit, dtrain[, 3:49])
knn_predict_test_res = predict(extree_fit, dtest[, 2:48])



write_csv(as.data.frame(knn_predict_train_res), "train_pred_targets_knn.csv")
write_csv(as.data.frame(knn_predict_test_res), "test_pred_targets_knn.csv")
