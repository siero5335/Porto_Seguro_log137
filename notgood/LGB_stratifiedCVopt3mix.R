...
lgb_cv_bayes <- function(lamnda_l1,
  ...
  cv <- lgb.cv(params = list(objective = "binary",  boosting_type = "gbdt", 
                             learning_rate = 0.001, tree_learner = "serial",
                             metric="auc",
                             max_depth = 8,
                             num_leaves = 2048, 
               ...
               data = dtrainreflgb, nrounds = 2000, 
               folds = cv_folds, early_stopping_rounds = 100, verbose = 0,
...
##lamnda_l1 = 0.0000	lambda_l2 = 0.3647	feature_fraction = 0.5021
##min_gain_to_split = 1.8361	Value = 0.6071 
...
params <- list(objective = "binary",  
               ...
               max_depth = 8,
               num_leaves = 2048, 
               min_gain_to_split = 1.3499,
               feature_fraction = 0.5021, 
               bagging_fraction = 0.6671,
               lamnda_l1 = 0,
               lambda_l2 = 0.3647,
               verbose = 0,
               categorical_feature = c(3, 5:11, 14:16, 20:29)
...
write.table(sub, "lgb_stratifiedCVBopt3mix.csv", sep=",", dec=".", quote=FALSE, row.names=FALSE)
...
write_csv(as.data.frame(train_pred_targets_lgb), "train_pred_targets_lgboost_stratifiedCVopt3mix.csv")
write_csv(as.data.frame(test_pred_targets_lgb), "test_pred_targets_lgboost_stratifiedCVopt3mix.csv")
